...
const defaultApplicationUri = (0, node_opcua_common_1.makeApplicationUrn)(node_os_1.default.hostname(), defaultProductUri);
let _discoveryDefaultCertificateManager;
function getDefaultCertificateManager() {
    if (!_discoveryDefaultCertificateManager) {
        ...
        _discoveryDefaultCertificateManager = new node_opcua_certificate_manager_1.OPCUACertificateManager({
            name: "PKI",
            rootFolder: node_path_1.default.join(config, "PKI"),
...
class OPCUADiscoveryServer extends node_opcua_server_1.OPCUABaseServer {
    ...
    async start() {
        ...
        const { hostname } = new node_url_1.URL(endpointUri);
    ...
    async #dealWithDiscoveryConfiguration(previousConfMap, server1, serverInfo, discoveryConfiguration) {
        ...
        const parsedUrl = new node_url_1.URL(endpointUrl);
    ...
    async #internalRegisterServer(RegisterServerXResponse, rawServer, discoveryConfigurations) {
        ...
        if (!_isValidServerType(server.serverType)) {
            debugLog("Invalid server Type", node_opcua_service_endpoints_1.ApplicationType[server.serverType]);
