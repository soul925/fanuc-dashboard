...
function verifyRankAndDimensions(options) {
    ...
    if (options.valueRank > 0 && options.arrayDimensions.length !== options.valueRank) {
        throw new Error(`[CONFORMANCE] when valueRank> 0, arrayDimensions must have valueRank elements, this.valueRank =${options.valueRank}  whereas arrayDimensions.length =${options.arrayDimensions?.length}`);
    }
    // c8 ignore stop
}
