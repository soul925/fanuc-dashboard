...
function encodeVariant(variant, stream) {
    ...
    if (variant.arrayType === VariantArrayType_enum_1.VariantArrayType.Matrix && dimensions && dimensions.length > 0) {
        ...
        const valueLength = variant.value?.length ?? 0;
        if (valueLength === 0) {
            dimensions = [];
        }
    }
    ...
    if (dimensions && variant.arrayType === VariantArrayType_enum_1.VariantArrayType.Matrix) {
        (0, node_opcua_assert_1.assert)(dimensions.length >= 0);
    ...
    if ((encodingByte & exports.VARIANT_ARRAY_DIMENSIONS_MASK) === exports.VARIANT_ARRAY_DIMENSIONS_MASK && dimensions) {
