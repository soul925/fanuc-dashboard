...
function _internal_getAllSubtypes(referenceType) {
    ...
    function _findAllSubType(referenceTypeInner) {
        if (!hasSubtypeReferenceType)
            throw new Error("cannot find HasSubtype reference type");
        possibleReferenceTypes.push(referenceTypeInner);
        (0, node_opcua_assert_1.assert)(referenceTypeInner.nodeClass === node_opcua_data_model_1.NodeClass.ReferenceType);
        ...
        for (const _r of references) {
            ...
            if (!subType)
                throw new Error("cannot find subtype reference type");
...
class UAReferenceTypeImpl extends base_node_impl_1.BaseNodeImpl {
    ...
    constructor(options) {
        ...
        this.inverseName =
            new node_opcua_data_model_1.LocalizedText(options.inverseName || this.browseName.name);
    ...
    readAttribute(context, attributeId) {
        ...
        switch (attributeId) {
            case node_opcua_data_model_1.AttributeIds.IsAbstract:
                ...
                break;
            case node_opcua_data_model_1.AttributeIds.Symmetric:
                options.value = { dataType: node_opcua_variant_1.DataType.Boolean, value: !!this.symmetric };
                options.statusCode = node_opcua_status_code_1.StatusCodes.Good;
                break;
            case node_opcua_data_model_1.AttributeIds.InverseName: // LocalizedText
