...
function getExtensionObjectArray(uaVariable) {
    const arr = uaVariable.$$extensionObjectArray;
    if (!arr) {
        throw new Error(`Internal Error: $$extensionObjectArray is not bound on UAVariable ${uaVariable.nodeId.toString()} (${uaVariable.browseName.toString()})`);
    }
    return arr;
}
...
function setExtensionObjectPartialValue(node, partialObject, sourceTimestamp) {
     ...
     */
    function _shouldRecurseIntoExisting(value) {
    ...
    function _shouldRecurseIntoNew(value) {
        if (value === null || value === undefined)
            return false;
        if (typeof value !== "object")
            return false;
...
function _installExtensionObjectBindingOnProperties(uaVariable, _options) {
...
function _installFields2(uaVariable, { get, set }, options) {
    ...
    for (const field of definition.fields || []) {
        ...
        propertyNode._inner_replace_dataValue = (dataValue, _indexRange) => {
            /** */
        ...
        if (propertyNode.dataTypeObj.basicDataType === node_opcua_variant_1.DataType.ExtensionObject) {
            _installFields2(propertyNode, {
                ...
                set: (fieldName, value, _sourceTime) => {
                    const mainFieldName = field.name;
        ...
        uaElement.$dataValue.value.arrayType = node_opcua_variant_1.VariantArrayType.Scalar;
        {
            ...
            _innerBindExtensionObjectScalar(uaElement, {
                ...
                set: (newValue, sourceTimestamp, cache) => {
                    ...
                    (0, node_opcua_assert_1.default)(!isProxy(extArray[capturedIndex]));
                ...
                setField: (fieldName, newValue, sourceTimestamp, cache) => {
                    ...
                    unProxy(extObj)[(0, node_opcua_utils_1.lowerFirstLetter)(fieldName)] = newValue;
                    propagateTouchValueUpward(capturedUaElement, sourceTimestamp, cache);
                }
            }, { ...options, force: true });
        }
