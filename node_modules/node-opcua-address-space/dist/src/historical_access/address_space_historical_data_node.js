...
class VariableHistorian {
    ...
    async push(newDataValue) {
        ...
        if (this._timeline.length >= this._maxOnlineValues || this._timeline.length === 1) {
            ...
            this.node._update_startOfOnlineArchive(first?.sourceTimestamp || node_opcua_date_time_1.minDate);
...
function _update_startOfOnlineArchive(newDate) {
    ...
    newDate = newDate || node_opcua_date_time_1.minDate;
    if (!newDate) {
...
function _historyPush(newDataValue) {
    ...
    (0, node_opcua_assert_1.assert)(Object.hasOwn(this, "historizing"), "expecting a historizing attribute on node");
    if (!this.historizing) {
        return; //
    }
...
function _historyReadModify(_context, _historyReadRawModifiedDetails, _indexRange, _dataEncoding, _continuationData, callback) {
    //
...
function _historyReadRaw(context, historyReadRawModifiedDetails, _indexRange, _dataEncoding, continuationData, callback) {
