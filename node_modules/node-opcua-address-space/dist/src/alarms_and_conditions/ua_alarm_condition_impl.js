...
class UAAlarmConditionImplBase extends ua_acknowledgeable_condition_impl_1.UAAcknowledgeableConditionImplBase {
    ...
    static instantiate(namespace, alarmConditionTypeId, options, data) {
        ...
        (0, node_opcua_assert_1.assert)(Object.hasOwn(options, "inputNode")); // must provide a inputNode
         ...
         */
        if (alarmNode.suppressedState) {
            alarmNode.suppressedState.on("value_changed", (_newDataValue) => {
                _update_suppressedOrShelved(alarmNode);
            });
        }
        if (alarmNode.shelvingState) {
            alarmNode.shelvingState.currentState.on("value_changed", (_newDataValue) => {
                _update_suppressedOrShelved(alarmNode);
            });
        }
        _update_suppressedOrShelved(alarmNode);
    ...
    _onInputDataValueChange(_newValue) {
        /**  */
    }
    /**
     ...
     *
     */
    _calculateConditionInfo(stateData, _isActive, value, _oldCondition) {
        if (!stateData) {
    ...
    _signalNewCondition(stateName, isActive, value) {
        ...
        (0, node_opcua_assert_1.assert)(!(0, node_util_1.isDeepStrictEqual)(oldConditionInfo, newConditionInfo), "condition values have not change, shall we really raise an event ?");
        if (isActive) {
