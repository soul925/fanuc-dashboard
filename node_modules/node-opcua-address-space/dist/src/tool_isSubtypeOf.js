...
function _filterSubType(reference) {
    return (0, node_opcua_nodeid_1.sameNodeId)(reference.referenceType, HasSubTypeNodeId) && !reference.isForward;
}
function _slow_isSubtypeOf(Class, baseType) {
    ...
    if ((0, node_opcua_nodeid_1.sameNodeId)(this.nodeId, baseType.nodeId)) {
        return true;
    }
    ...
    for (const subType1 of subTypes) {
        ...
        if ((0, node_opcua_nodeid_1.sameNodeId)(subTypeNode.nodeId, baseType.nodeId)) {
            return true;
        }
        else {
...
function wrap_memoize(func, hashFunc) {
    const effectiveHashFunc = hashFunc ??
        function (param) {
            return param.toString();
        };
    return function memoize(param) {
        let memoMap = g_WeakMap.get(this);
        if (!memoMap) {
            memoMap = new Map();
            g_WeakMap.set(this, memoMap);
        }
...
function hashBaseNode(e) {
    ...
    return (0, node_opcua_nodeid_1.resolveNodeId)(e).toString();
}
...
function get_subtypeOfObj() {
    ...
    return _cache._subtypeOfObj || null;
}
