...
const _warningLog = (0, node_opcua_debug_1.make_warningLog)(__filename);
exports.NamespaceOptions = {
    nodeIdNameSeparator: "-"
};
function _isValidNodeClass(nodeClass) {
...
function _filterAggregates(addressSpace, references) {
    const aggregatesRefType = addressSpace.findNode((0, node_opcua_nodeid_1.resolveNodeId)("Aggregates"));
    const hasEncodinRefType = addressSpace.findNode((0, node_opcua_nodeid_1.resolveNodeId)("HasEncoding"));
    const checkAggregate = (reference) => {
        if (reference.isForward)
            return false;
        const r = (0, reference_impl_1.resolveReferenceType)(addressSpace, reference);
        if (!r) {
            return false;
        }
    ...
    (0, node_opcua_assert_1.assert)(candidates.length <= 1, "a node shall not have more than one parent (link to a parent with a reference derived from 'Aggregates')");
...
class NodeIdManager {
    _cacheSymbolicName = {};
    _cacheSymbolicNameRev = new Set();
    _internal_id_counter;
    namespaceIndex;
    addressSpace;
    ...
    getSymbols() {
        ...
        for (const [key, [value, nodeClass1]] of Object.entries(this._cacheSymbolicName)) {
            const _node = this.addressSpace.findNode((0, node_opcua_nodeid_1.makeNodeId)(value, this.namespaceIndex));
    ...
    constructNodeId(options) {
        ...
        if (!options.nodeId && options.registerSymbolicNames) {
            ...
            if (cached && this._isCacheHitReusable(cached[0], options, parentInfo)) {
                return (0, node_opcua_nodeid_1.makeNodeId)(cached[0], this.namespaceIndex);
            }
            const nodeId = this._constructNodeId(options);
            if (nodeId.identifierType === node_opcua_nodeid_1.NodeIdType.NUMERIC && !cached) {
                this._cacheSymbolicName[fullName] = [nodeId.value, options.nodeClass || node_opcua_data_model_1.NodeClass.Unspecified];
        ...
        if (options.registerSymbolicNames && nodeId.identifierType === node_opcua_nodeid_1.NodeIdType.NUMERIC) {
            const parentInfo = this.findParentNodeId(options);
            let fullParentName = "";
            if (parentInfo) {
                const [parentNodeId, suffix] = parentInfo;
     ...
     */
    _isCacheHitReusable(value, options, parentInfo) {
        const candidate = (0, node_opcua_nodeid_1.makeNodeId)(value, this.namespaceIndex);
        ...
        const sameBrowseName = !!occupant.browseName &&
        ...
        const sameParent = expectedParentNodeId
            ? !!occupant.parentNodeId && (0, node_opcua_nodeid_1.sameNodeId)(occupant.parentNodeId, expectedParentNodeId)
            : !occupant.parentNodeId;
        return sameBrowseName && sameParent;
    }
    _constructNodeId(options) {
        const resolveNodeIdEx = (nodeId) => nodeId && typeof nodeId === "object" && nodeId instanceof base_node_impl_1.BaseNodeImpl
            ? nodeId.nodeId
            : (0, node_opcua_nodeid_1.resolveNodeId)(nodeId);
    ...
    _isInCache(nodeId) {
        if (nodeId.namespace !== this.namespaceIndex || nodeId.identifierType !== node_opcua_nodeid_1.NodeIdType.NUMERIC)
            return false;
        return !!this._cacheSymbolicNameRev.has(nodeId.value);
    }
}
