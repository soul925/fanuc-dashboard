...
function _dumpVariantValue(xw, dataTypeNodeId, dataType, addressSpace, value) {
...
function _dumpVariantInnerExtensionObject(xw, definitionMap, definition, addressSpace, value) {
    const namespaceUri = findXsdNamespaceUri(xw, definition.defaultEncodingId);
    const ns = getPrefix(xw, namespaceUri);
    const isUnion = definition.structureType === node_opcua_types_1.StructureType.Union || definition.structureType === node_opcua_types_1.StructureType.UnionWithSubtypedValues;
    for (const field of definition.fields || []) {
        const dataTypeNodeId = field.dataType;
        const fieldName = field.name;
        const lowerFieldName = (0, node_opcua_utils_1.lowerFirstLetter)(fieldName);
...
function _dumpVariantInnerValue(xw, dataType, dataTypeNodeId, definitionMap, addressSpace, value) {
    const uax = getPrefix(xw, "http://opcfoundation.org/UA/2008/02/Types.xsd");
    switch (dataType) {
        case null:
        ...
        case node_opcua_variant_1.DataType.ExtensionObject:
            _dumpVariantExtensionObjectValue(xw, dataTypeNodeId, definitionMap, addressSpace, value);
            break;
 ...
 * @private
 */
function _dumpVariantExtensionObjectValue_Body(xw, definitionMap, name, definition, addressSpace, value) {
    if (value) {
        const namespaceUri = findXsdNamespaceUri(xw, definition.defaultEncodingId);
        const ns = getPrefix(xw, namespaceUri);
...
function _dumpVariantExtensionObjectValue(xw, dataTypeNodeId, definitionMap, addressSpace, value) {
    const { name, definition } = definitionMap.findDefinition(dataTypeNodeId);
...
function _dumpVariantExtensionObjectValue2(xw, addressSpace, value) {
    ...
    if (value.arrayType === node_opcua_variant_1.VariantArrayType.Scalar) {
        switch (value.dataType) {
            case node_opcua_variant_1.DataType.ExtensionObject:
                ...
                break;
            case node_opcua_variant_1.DataType.DateTime:
                if (!value.value || (0, node_opcua_basic_types_1.isMinDate)(value.value)) {
                    return true;
                }
                break;
            case node_opcua_variant_1.DataType.ByteString:
...
function _dumpValue(xw, node, variant) {
    const addressSpace = node.addressSpace;
    // c8 ignore next
    ...
    const isExtensionObject = variant.dataType === node_opcua_variant_1.DataType.ExtensionObject;
        ...
        for (const reference of r) {
            ...
            if (nodeChild.nodeId.namespace === node.nodeId.namespace) {
                if (!xw.visitedNode.has(_hash(nodeChild))) {
                    debugLog(node.nodeId.toString(), " dumping child ", nodeChild.browseName.toString(), nodeChild.nodeId.toString());
...
function dumpCommonAttributes(xw, node) {
    ...
    if (node.nodeClass === node_opcua_data_model_1.NodeClass.Variable && node.historizing) {
...
function _dumpEnumDefinition(xw, enumDefinition) {
    ...
    for (const defItem of enumDefinition.fields) {
        ...
        if (!(0, node_opcua_utils_1.isNullOrUndefined)(defItem.value)) {
...
function _dumpStructureDefinition(xw, structureDefinition, baseStructureDefinition) {
    ...
    const nbFieldsInBase = baseStructureDefinition ? baseStructureDefinition.fields?.length || 0 : 0;
...
function _markAsVisited(xw, node) {
    ...
    (0, node_opcua_assert_1.assert)(!xw.visitedNode.has(_hash(node)));
...
ua_view_impl_1.UAViewImpl.prototype.dumpXML = function (xw) {
    dumpUAView(xw, this);
};
