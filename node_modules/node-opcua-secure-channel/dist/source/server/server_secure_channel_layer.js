...
var nonce_cache_1 = require("./nonce_cache");
Object.defineProperty(exports, "isEmptyNonce", { enumerable: true, get: function () { return nonce_cache_1.isEmptyNonce; } });
Object.defineProperty(exports, "nonceAlreadyBeenUsed", { enumerable: true, get: function () { return nonce_cache_1.nonceAlreadyBeenUsed; } });
const nonce_cache_2 = require("./nonce_cache");
/**
 */
class ServerSecureChannelLayer extends node_events_1.EventEmitter {
    ...
    toString() {
        return `ServerSecureChannelLayer({ channelId: ${this.channelId}, securityMode: ${node_opcua_service_secure_channel_1.MessageSecurityMode[this.securityMode]}, securityPolicy: ${this.securityPolicy}, remoteAddress: ${this.remoteAddress}, remotePort: ${this.remotePort} })`;
    }
     ...
     *
     */
    getEndpointDescription(_securityMode, securityPolicy, endpointUri) {
        if (!this.#parent) {
        ...
        this.#transport.init(socket, (err) => {
            ...
            if (err) {
                ...
                this.#transport.on("chunk", (messageChunk) => {
                    // c8 ignore next
                    if (node_opcua_transport_1.doTraceIncomingChunk) {
    ...
    send_response(msgType, response, message, callback) {
        ...
        if (statusCode.isNotGood()) {
            if (response instanceof services_1.ServiceFault) {
                ...
                errorLog("send_response: failed to send ServiceFault, " + "aborting to prevent infinite recursion. " + "statusCode =", statusCode.toString());
            ...
            this.send_response(msgType, new services_1.ServiceFault({
    ...
    #_sendFatalErrorAndAbort(statusCode, description, _message, callback) {
    ...
    #_install_wait_for_open_secure_channel_request_timeout(timeoutFoOpenChannelRequest) {
        ...
        this.#open_secure_channel_onceClose = (_err) => {
            // c8 ignore next
            if (doDebug) {
                debugLog("_install_wait_for_open_secure_channel_request_timeout:");
    ...
    #_on_initial_open_secure_channel_request(request, requestId, channelId) {
        ...
        /* c8 ignore next */
        if (!(this.#messageBuilder?.sequenceHeader && this.#messageBuilder.securityHeader)) {
            ...
            return this.#_on_OpenSecureChannelRequestError(node_opcua_status_code_1.StatusCodes.BadCommunicationError, "internal error", {
                request,
                requestId,
                securityHeader
            });
        ...
        this.#messageBuilder
            ...
            .on("error", (err, statusCode, _requestId) => {
            /** */
            this.#transport.sendErrorMessage(statusCode, err.message);
            this.close(() => {
                /** */
    ...
    #_wait_for_open_secure_channel_request(timeout) {
        ...
        const messageHandler = (request, _msgType, _securityHeader, requestId, channelId) => {
            this.#_cancel_wait_for_open_secure_channel_request_timeout();
    ...
    #_get_security_options_for_MSG(tokenId) {
        ...
        if (!derivedKeys || !derivedKeys.derivedServerKeys) {
            ...
            if (fallbackToken) {
                warningLog(`tokenId ${tokenId} expired — falling back to` +
                    ` latest valid token ${fallbackToken.tokenId}` +
                    ` (securityMode=${node_opcua_service_secure_channel_1.MessageSecurityMode[this.securityMode]})`);
                derivedKeys = fallbackToken.derivedKeys;
            }
            else {
                errorLog(`no valid token available at all, securityMode=${node_opcua_service_secure_channel_1.MessageSecurityMode[this.securityMode]}`);
                return null;
            }
        }
    ...
    #_process_certificates(message, callback) {
        ...
        this.checkCertificate(clientCertificate)
            ...
            if (statusCode.isNotGood()) {
                ...
                const chain = (0, web_1.split_der)(clientCertificate || Buffer.alloc(0));
                warningLog("Sender Certificate = ", chain.map((c) => (0, common_1.getThumbprint)(c)?.toString("hex")).join("\n"));
            }
            this.#clientCertificate = null;
            this.#clientPublicKey = null;
            this.#clientPublicKeyLength = 0;
                ...
                (0, web_1.extractPublicKeyFromCertificate)(clientCertificate, (err, keyPem) => {
                    if (!err) {
                        if (keyPem) {
                            ...
                            this.#clientPublicKey = (0, node_crypto_1.createPublicKey)(keyPem);
    ...
    #_prepare_response_security_header(request, message) {
        ...
        switch (request.securityMode) {
            case node_opcua_service_secure_channel_1.MessageSecurityMode.Invalid:
                ...
                securityHeader = new services_1.AsymmetricAlgorithmSecurityHeader({
                    ...
                    securityPolicyUri: security_policy_1.SecurityPolicy.Invalid,
    ...
    #_check_client_nonce(clientNonce) {
        if (this.securityMode !== node_opcua_service_secure_channel_1.MessageSecurityMode.None) {
            ...
            /* c8 ignore next */
            if ((0, nonce_cache_2.nonceAlreadyBeenUsed)(clientNonce)) {
    ...
    #_make_serverNonce() {
        if (this.securityMode !== node_opcua_service_secure_channel_1.MessageSecurityMode.None && this.#messageBuilder) {
            const cryptoFactory = (0, security_policy_1.getCryptoFactory)(this.#messageBuilder.securityPolicy);
            ...
            return (0, node_opcua_utils_1.randomBytes)(cryptoFactory?.symmetricKeyLength || 0);
        }
        return null;
    }
    #_make_derivedKeys(serverNonce, clientNonce) {
        if (this.securityMode !== node_opcua_service_secure_channel_1.MessageSecurityMode.None) {
            // c8 ignore next
            if (!serverNonce || !clientNonce || !this.#messageBuilder) {
    ...
    #_handle_OpenSecureChannelRequest(messageRequest) {
        this.beforeHandleOpenSecureChannelRequest().then(() => {
            ...
            if (!messageRequest.securityHeader) {
                return this.#_on_OpenSecureChannelRequestError(node_opcua_status_code_1.StatusCodes.BadSecurityModeRejected, "invalid security header", messageRequest);
            }
            const request = messageRequest.request;
            const requestId = messageRequest.requestId;
            /* c8 ignore next */
            if (request.securityMode === node_opcua_service_secure_channel_1.MessageSecurityMode.Invalid) {
                return this.#_on_OpenSecureChannelRequestError(node_opcua_status_code_1.StatusCodes.BadSecurityModeRejected, "invalid security mode", messageRequest);
            }
